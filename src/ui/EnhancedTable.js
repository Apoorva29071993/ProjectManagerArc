...
export default function EnhancedTable(props) {
  ...
  const switchFilters = () => {
    const {websiteChecked , iOSChecked , androidChecked ,
           customSoftwareChecked} = props;

    const websites = props.rows.filter(row => websiteChecked ? 
      row.service === "Website" : null)  ;   

    const iOSApps = props.rows.filter(row => iOSChecked ? 
        row.platforms.includes("iOS") : null)  ; 

    const androidApps = props.rows.filter(row => androidChecked ? 
          row.platforms.includes("Android") : null)   ; 

    const softwareApps = props.rows.filter(row => customSoftwareChecked ? 
            row.service === "Custom Software" : null)  ;   
    
    if (!websiteChecked && !iOSChecked && 
      !androidChecked && !customSoftwareChecked) {
        return props.rows
        }   
     else {
        let newRows = websites.concat(iOSApps.filter(item => 
          websites.indexOf(item) < 0));
        let newRows2 = newRows.concat(androidApps.filter(item => 
            newRows.indexOf(item) < 0));
        let newRows3 = newRows2.concat(softwareApps.filter(item => 
            newRows2.indexOf(item) < 0));
